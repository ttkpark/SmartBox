...
void loop() {
  ...
  Port_uartprocessing();
  //FFSWebloop();
  //println("Port_uartprocessing() end");
  MSKeyloop();
  ...
  if(TransmitINT)
  {
    Process_Access("sbkkb000000009");
    Process_getkey();
    if(Infobuflen>1)
      RtnToSerial("NDT","01");
    else//1 byte : Cancel    0 byte : Fail
      RtnToSerial("NDT","00");
    delay(100);
    TransmitINT = 0;
  }
  /*
    ...
    PORTC = (PORTC&(~0x01))|!(PORTC&0x01);
    uint8_t data = spi_transfer(0x11);
