...
extern CSmartBoxMasterKeyAppDlg* MainDlg;
...
int CMycomm::open(CString _ComPortName)
{
	if(CommOpened)
	{
		Msgstr = GetCaption(54);//ID54 Open Error : COM Port is already opened!!
...
/*
	...
	{
		Msgstr = GetCaption(55);//ID55 Open Error : COM Port Connect Setting Fail!!
		::PostMessage(hCommWnd, WM_STATBARTXT, (WPARAM)(&Msgstr), NULL);
...
int CMycomm::close() {
	...
	{
		Msgstr = GetCaption(56);//ID56 Close Error : COM Port is NOT opened!!
		::PostMessage(hCommWnd, WM_STATBARTXT, (WPARAM)(&Msgstr), NULL);
		return -1;
	}
...
int CMycomm::write(void* WriteData, int _Length = -1) {
	...
	if (!CommOpened)
	{
		Msgstr = GetCaption(57);//ID57 Write Error : COM Port is not connected!!
		::PostMessage(hCommWnd, WM_STATBARTXT, (WPARAM)(&Msgstr), NULL);
		return -1;
	}
...
int CMycomm::read(void* ReadData, DWORD Length) {
	if (!CommOpened)
	{
		Msgstr = GetCaption(58);//ID58 Read Error : COM Port is not connected!!
		::PostMessage(hCommWnd, WM_STATBARTXT, (WPARAM)(&Msgstr), NULL);
		return -1;
	}
...
DWORD	ThreadWatchComm(CMycomm* pComm)
{
	...
	if (!bOk)
	{
		Msgstr.Format(GetCaption(59), pComm->m_sPortName);//ID59 Error while creating ThreadWatchComm, %s
		::PostMessage(hCommWnd, WM_STATBARTXT, (WPARAM)(&Msgstr), NULL);
		return FALSE;
	}
		...
		if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
		{
			...
			do
			{
				...
				for (int i = 0; i < dwRead; i++)
					flag |= buff[i];
				if (flag == 0)//if therre's no message
					continue;
				...
				else
				{
					...
					Msgstr = GetCaption(60);//ID60 buff full
					::PostMessage(hCommWnd, WM_STATBARTXT, (WPARAM)(&Msgstr), NULL);
				}
			} while (dwRead);
...
//ID54 Open Error  : COM Port is already opened!!
//ID55 Open Error  : COM Port Connect Setting Fail!!
//ID56 Close Error  : COM Port is NOT opened!!
//ID57 Write Error : COM Port is not connected!!
//ID58 Read Error  : COM Port is not connected!!
//ID59 Error while creating ThreadWatchComm, %s
//ID60 buff full