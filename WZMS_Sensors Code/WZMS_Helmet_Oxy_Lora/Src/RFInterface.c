...
/**
  ******************************************************************************
  * @file           : RFInterface.c
  * @brief          : RF Interface program
  ******************************************************************************
  * @attention
  *
  * <h2><center>&copy; Copyright (c) 2019 GH
...
//extern char* DeviceSNM;// = "wzkkh000000001";
...
int RFsend()
{
  ...
  while(Start)
  {
    ...
    if(Start)
    {
      ...
      DebugRFn("TX 1-1 : %s",printBin(rx_address,5));
      ...
      DebugRF("Start~");
      int count = data_array[1], remain = data_array[2], len = count<<5 | remain, i=0;
      //DebugRF("count:%d,remain:%d,len:%d",count,remain,len);
      ...
      while(i!=count)
      {
        ...
        //Recv N-1
        if(WZRF_Rx()){DebugRF("Timeout : --ms.%d",__LINE__);Start=2;break;}
      ...
      time = micros()-time;
      DebugRF("TX LN-1: ACK");
      println("len : %d, count : %d, remain : %d",len,count,remain);
      println("Data : %s",jsondata);
      //free(temp);

      int mill = time/1000;
      int micr = time - mill*1000;
      println("Sensing time : %d.%03dms",mill,micr);
      Start=3;
      */
...
int WZRF_Rx()
{
  ...
  nrf24_powerUpRx();
  uint32_t tmpTime = WZMS_millis();
  while(!nrf24_dataReady() && WZMS_millis()-tmpTime < 1000);//delay_ms(1);//RX 1-2
...
{
	//HAL_GPIO_WritePin(NRF_CE_GPIO_Port,NRF_CE_Pin,pin);
  NRF_CE_GPIO_Port->BSRR = pin?NRF_CE_Pin:NRF_CE_Pin<<16;
}
void nrf24_csn_digitalWrite(int pin)
{
	//HAL_GPIO_WritePin(NRF_CSN_GPIO_Port,NRF_CSN_Pin,pin);
  NRF_CSN_GPIO_Port->BSRR = pin?NRF_CSN_Pin:NRF_CSN_Pin<<16;
}
uint8_t spi_transfer(uint8_t dat)
{
	uint8_t out = 0;
	HAL_SPI_TransmitReceive(NRFSPI, &dat, &out, sizeof(dat), 0x100);
	return out;
}
void spi_transfers(uint8_t *tx,uint8_t *rx, int len)
{
  uint8_t *buf,*in,*out;
  if(!tx | !rx)buf = (uint8_t*)malloc(len);
  if(tx!=0)in = tx;else in = buf;
  if(tx!=0)out = rx;else out  =buf;
  HAL_SPI_TransmitReceive(NRFSPI, in, out, len, 0x100);
  if(!tx | !rx)free(buf);
}