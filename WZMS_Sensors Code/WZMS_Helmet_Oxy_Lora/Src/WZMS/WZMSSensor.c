...
// int Command_SensorOnOff(char* Command)
// {
//   char UARTBuffer[7];
//   HAL_UART_Transmit(SensorUart,Command,strlen(Command),1000);
//   int available = uart_available();
//   //while(uart_available() < available + 6)println("%d",uart_available());
//   HAL_Delay(100);
//   int retlen = 6;
//   for(int i=0;i<6;i++)
//   {
//     UARTBuffer[i] = SeBuf[i];
//   }
//   //HAL_UART_Receive(SensorUart,UARTBuffer,6,1000);
//   println("CommandData : %s",printBin(UARTBuffer[3],6));
  
//   //Response from Sensor : 'M' ' ' 'x' 'x' '\r' '\n' (6-Bytes)
//   if(UARTBuffer[4] == '\r' && UARTBuffer[5] == '\n')//has Received well?
//   {
//     if(Command[3] == UARTBuffer[3])
//     {
//       return 0;//RES_OK
//     }
//     else println("Data is not Correct!! '%c' = '%c'",Command[3],UARTBuffer[3]);
//   }
//   return 1;//RES_ERR
// }
// int Sensor_On()
// {
//   return Command_SensorOnOff("M 0\r\n");
// }
// int Sensor_Off()
// {
//   return Command_SensorOnOff("M 2\r\n");
// }
uint32_t t1 = 0;
volatile uint8_t isOxySensorOn = true;
void WZMSSensor_processing(int process){
  ...
  //println("WZMSInterval : %d , process : %d , diff : %d",WZMSInterval,process,WZMSInterval-process);
  if(process == -1){
    float bt = 0;
    float volt = 0;
    volt = read_Vcc();
    ...
    ((float*)(PortData[0]))[0] = *((float*)&(oxydata[16]));//ox
    ((float*)(PortData[1]))[0] = *((float*)&(oxydata[0]));//tp
    //((float*)(PortData[2]))[0] = *((float*)&(oxydata[32]));//ps
    ((float*)(PortData[3]))[0] = bt;//bt
